...
// [[Rcpp::export]]
Rcpp::List adaptive_mh(const arma::vec& radii, const arma::vec& angles, 
  ...
  double prior = prior_fn_radii(theta);
    double likelihood = radial_top5_loglik(radii, theta, angles, gauge_type);
      ...
      for(int iter = 1; iter < n_updates; ++iter) {
        ...
        if(prior_star != -datum::inf) {
          double likelihood_star = radial_top5_loglik(radii, theta_star, angles, gauge_type);
  ...
  mat result_matrix(thinning_idx.size(), 3);
  ...
  Rcpp::CharacterVector col_names = {"alpha", "dep", "sigma_m"};
